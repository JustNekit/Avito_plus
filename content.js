window.onload = function () {
  ...
  function addImageToSmallCard(announcement) {
    var mainMarker = announcement.querySelector('[class="styles-link-cQMwi"]');
    // var div = document.createElement("div");
    // var p = mainMarker;
    // div.appendChild(p);
    if (mainMarker) {
      // Set background-position: right; and remove width for favorites-add
      // mainMarker.style.backgroundPosition = "left";
      // mainMarker.style.width = "auto";

      // Create a container for images
      var imageContainer = document.createElement("div");
      imageContainer.style.position = "absolute"; // Set the container position to relative
      ...
      imageContainer.style.right = "0px";
      imageContainer.style.borderTopLeftRadius = "50px";
      imageContainer.style.borderBottomLeftRadius = "50px";
      ...
      mainMarker.parentNode.insertBefore(imageContainer, mainMarker.nextSibling);
        ...
        headers: {
          ...
          .then(res => {
            if(res.code==200){
              ...
              var avgPrice = res.avgPrice.toFixed(0)
              info.textContent = "Средняя цена = "+avgPrice



              var ppp =  announcement.querySelector("[itemprop=price]")
            ...
            console.log(res.avgPrice)
            console.log(res.similarAds.length)
          });
